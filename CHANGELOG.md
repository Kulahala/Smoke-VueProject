...
- **2026-06-07**:
  - **菜单与下拉浮窗磨砂玻璃化统一**：升级了 PC 端“产品”下拉菜单（`.nav-menu`）、“偏好设置”浮窗（`.utility-panel`）及移动端“菜单抽屉”（`.nav-shell`）的视觉系统，全部统一为与顶部导航栏一致的 `backdrop-filter: blur(18px)` 磨砂玻璃特效；并在小屏响应式下自动消除嵌套浮窗的磨砂背景与边框，防止多层堆叠叠加。
